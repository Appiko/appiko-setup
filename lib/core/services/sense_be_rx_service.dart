...
class SenseBeRxService extends ChangeNotifier {
  ...
  Setting get activeSetting => structure.settings[activeSettingIndex];
  ...
  setActiveIndex({int tabIndex, Setting setting}) {
    ...
    if (setting == null) {
      ...
      Setting s = structure.settings.firstWhere(
      ...
      structure.settings[activeSettingIndex] =
          Setting.clone(setting: setting, index: 8);
  ...
  void updateSetting() {
    ...
    metaStructure.advancedOptionsEnabled[settingToUpdate] =
        metaStructure.advancedOptionsEnabled[8];
    structure.settings[8] = Setting(index: 8);
  ...
  deleteSetting(Setting setting) {
    ...
    structure.settings[setting.index] = Setting(index: setting.index);
  ...
  deleteAllSettings(Type type) {
    structure.settings = structure.settings.map((setting) {
      if (setting.sensorSetting.runtimeType == type) {
        print("Deleting");
        subtactStateIfAmbient(setting);
        return Setting(index: setting.index);
      }
      return setting;
    }).toList();
  ...
  String timeCheck({int pickerEndTime, int pickerStartTime, int settingIndex}) {
    ...
    for (int i = 0; i < 8; i++) {
      Setting setting = structure.settings[i];
