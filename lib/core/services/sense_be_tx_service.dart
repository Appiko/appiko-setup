...
import 'package:setup/ui/devices/sense_be_tx/1.0/settings/half_press_settings_view.dart';
import 'package:setup/ui/devices/sense_be_tx/1.0/settings/long_press_settings.dart';
import 'package:setup/ui/devices/sense_be_tx/1.0/settings/multiple_pictures_settings_view.dart';
import 'package:setup/ui/devices/sense_be_tx/1.0/settings/single_picture_settings_view.dart';
import 'package:setup/ui/devices/sense_be_tx/1.0/settings/timer_settings_view.dart';
import 'package:setup/ui/devices/sense_be_tx/1.0/settings/video_settings_view.dart';
...
/// {@category SenseBeRx}
/// {@category Algorithm}
class SenseBeTxService extends ChangeNotifier {
  ...
  BeTxSetting get activeSetting => structure.settings[activeSettingIndex];
  ...
  getSensorView({context}) {
    if (activeTriggerType == TriggerType.MOTION_TRIGGER) {
      setMotion(downtime: 0, numberOfTriggers: 0, sensitivity: 0);
      Navigator.popUntil(
          context, ModalRoute.withName(getCameraSettingDownArrowPageName()));
      return null;
    } else {
  ...
  setActiveIndex({int tabIndex, BeTxSetting setting}) {
    ...
    if (setting == null) {
      ...
      structure.settings[activeSettingIndex] =
          BeTxSetting.clone(setting: setting, index: 8);
  ...
  void updateSetting() {
    ...
    metaStructure.advancedOptionsEnabled[settingToUpdate] =
        metaStructure.advancedOptionsEnabled[8];
    structure.settings[8] = BeTxSetting(index: 8);
  ...
  closeFlow() {
    if (!settingFlowComplete && !shouldPassSetting) {
      structure.settings[activeSettingIndex] =
          BeTxSetting(index: activeSettingIndex);
      notifyListeners();
    }
  }
  ...
  String getCameraSettingDownArrowPageName() {
    if (shouldPassSetting) {
      return '/bt/setting-summary';
    }
    return getCloseSummaryPath();
  }

  deleteSetting(BeTxSetting setting) {
    if (setting.index == 8) {
      setting = structure.settings[settingToUpdate];
    }
    structure.settings[setting.index] = BeTxSetting(index: setting.index);
  ...
  deleteAllSettings(Type type) {
    structure.settings = structure.settings.map((setting) {
      if (setting.sensorSetting.runtimeType == type) {
        print("Deleting");
        subtactStateIfAmbient(setting);
        return BeTxSetting(index: setting.index);
      }
      return setting;
    }).toList();
  ...
  void discardChangeFlow() {
    structure.settings[8] =
        BeTxSetting.clone(setting: structure.settings[settingToUpdate]);
  }
