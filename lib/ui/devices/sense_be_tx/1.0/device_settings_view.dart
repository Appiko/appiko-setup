...
class _DeviceSettingsViewState extends State<DeviceSettingsView>
  ...
  void initState() {
    ...
    locator<SenseBeTxService>().reset();
    super.initState();
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (Provider.of<SenseBeTxService>(context).structure.operationTime[0] ==
        OperationTime.ALL_TIME) {
      motionFABDisabledMessage = allTimeErrorMessage;
    } else if (Provider.of<SenseBeTxService>(context)
            .structure
            .motionAmbientState ==
        7) {
    ...
    if (Provider.of<SenseBeTxService>(context).structure.operationTime[1] ==
        OperationTime.ALL_TIME) {
    ...
    if (deviceState == BluetoothDeviceState.connected && _isBluetoothOn) {
      _memoizer.runOnce(() {
        locator<SenseBeTxService>().readFromDevice();
      });
    }
    return Provider.of<SenseBeTxService>(context).deviceInfo == null
        ? Scaffold(
            body: Center(
              child: Column(
                mainAxisAlignment: MainAxisAlignment.center,
                    ...
                    ? FloatingActionButton(
                        ...
                        onPressed: (_tabController.index == 0 &&
                                ...
                                (_tabController.index == 1 &&
                                    timerFABDisabledMessage.isEmpty)
                            ? () {
                                locator<SenseBeTxService>().setActiveIndex(
  ...
  closeConnection(BuildContext context, bool isDisconnected) {
    ...
    if (locator<SenseBeTxService>().shouldSave && !isDisconnected) {
      showDialog(
        context: context,
      ...
      headerSliverBuilder: (BuildContext context, bool boxIsScrolled) {
        return <Widget>[
          SliverAppBar(
            ...
            title: Text(
              locator<SenseBeTxService>().structure.deviceName,
              style: TextStyle(
              ...
              IconButton(
                ...
                onPressed: () {
                  TextEditingController name = TextEditingController(
                    text: Provider.of<SenseBeTxService>(context)
                        .structure
                        .deviceName
                        .trim(),
                  );
