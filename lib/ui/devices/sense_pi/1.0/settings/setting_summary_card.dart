...
class SettingSummaryCard extends StatelessWidget {
  ...
  Future<void> showAmbientOptionsBottomSheet({
    ...
    if (setting.sensorSetting.runtimeType == MotionSetting) {
      state = locator<SensePiService>().structure.motionAmbientState;
    } else {
      state = locator<SensePiService>().structure.timerAmbientState;
    }
    locator<AmbientFieldsModel>().disabledFields =
        locator<SensePiService>().getModesToBeDisabled(
      state: state,
        ...
        Container(
          ...
          child: CupertinoDatePicker(
            ...
            onDateTimeChanged: (val) {
              setState(() {
                t = val;
                errorMessage = locator<SensePiService>().timeCheck(
                    settingIndex: widget.settingIndex,
                    pickerStartTime: time.getTimeInSeconds(t),
                    pickerEndTime: (locator<SensePiService>()
                            .structure
                ...
                if (widget.isStartTime) {
                  ...
                  errorMessage = locator<SensePiService>().timeCheck(
                      ...
                      pickerStartTime: (locator<SensePiService>()
                              .structure
                              .settings[widget.settingIndex]
                              .time as time.TimeOfDay)
                          .startTime);
